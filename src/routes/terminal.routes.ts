...
import { backofficeMiddleware, commerceMiddleware, franchiseMiddleware } from '@app/middlewares/auth.middleware'
...
class UserRoutes extends ServerRouter {
  ...
  config (): void {
    this.router.get('/', terminalController.getTerminals as RequestHandler)
    this.router.get('/resume', terminalController.terminalCount as RequestHandler)
